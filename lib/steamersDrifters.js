...
export async function fetchSteamerDrifterFlags({ venue, raceNum, date }) {
  ...
  // Distinct captured_at values for this race today, ascending -- a single
  // race's poll history for one day is small (~15min cadence), so no
  // pagination concern here (unlike a whole day's odds_snapshot).
  const tsRows = await sb(
    `odds_snapshot?race_date=eq.${date}&race_venue=eq.${encodeURIComponent(venue)}&race_num=eq.${encodeURIComponent(raceNum)}&select=captured_at&order=captured_at.asc`,
  );
