...
export const HEADER_ALIASES = [
  ...
  ['Career Prizemoney', ['career prizemoney','career prize money','career earnings']],
  ...
  ['Trn Ovrl12m Wins',['trn ovrl12m wins','trn overall12m wins','trn overall 12m wins','trainer overall wins']],
  ['Trn Ovrl12m Starts',['trn ovrl12m starts','trn overall12m starts','trn overall 12m starts','trainer overall starts']],
  ...
  ['Jockey',       ['jockey','joc']],
  ['Trainer',      ['trainer','trn']],
  ['state',        ['state','venue state','country','jurisdiction']],
...
// Deterministic ±1-2% jitter seeded from stable per-runner identifiers (djb2
// string hash) — same horse/race/date always produces the same multiplier,
// regardless of how many times the CSV is re-parsed or the page reloaded,
// while different horses land at different points in the range.
function seededJitter(seed) {
  let hash = 5381;
  for (let i = 0; i < seed.length; i++) {
    hash = ((hash << 5) + hash) + seed.charCodeAt(i); // hash * 33 + c
    hash |= 0; // keep to 32 bits
  }
  const normalized = (hash >>> 0) / 4294967295; // 0..1
  return 0.98 + normalized * 0.04; // 0.98..1.02
}
...
const NZ_TRACKS = new Set([
  // Major metropolitan / provincial
  'ellerslie', 'trentham', 'awapuni', 'riccarton', 'te rapa', 'hastings',
  'otaki', 'rotorua', 'matamata', 'pukekohe', 'wanganui', 'wingatui',
  'ashburton', 'ruakaka', 'tauherenikau', 'cambridge',
  // Regional thoroughbred tracks
  'te aroha', 'tauranga', 'taupo', 'gore', 'timaru', 'oamaru',
  'kurow', 'waimate', 'winton', 'wairoa', 'woodville', 'waipukurau',
  'dannevirke', 'foxton', 'levin', 'avondale', 'te awamutu', 'paeroa',
  'thames', 'whangarei', 'dargaville', 'stratford', 'hawera', 'new plymouth',
  'marton', 'feilding', 'waverley',
  // South Island
  'greymouth', 'reefton', 'westport', 'omakau', 'cromwell', 'wyndham',
  'riverton', 'invercargill', 'ascot park', 'bluff',
  // Synthetic / branded tracks
  'phar lap raceway', 'hutt park',
]);

// Blocklist-based, not allowlist: a meeting is excluded only if its venue
// name matches a known NZ track. An allowlist was tried instead (require a
// venue to resolve to a known AU venue) and reverted -- it excluded real AU
// meetings (Moe, Wellington) that were never added to the allowlist, which
// is a worse failure mode than an obscure NZ meeting occasionally slipping
// through this blocklist. A real AU venue silently vanishing is more
// damaging than a rare unlisted NZ meeting silently appearing.
//
// To compensate for the blocklist's blind spot (it can only ever exclude
// tracks someone thought to enumerate), see the today_meetings cross-check
// in app/api/upload-race-csv/route.js and app/api/import-csv/route.js: it
// doesn't exclude anything here, but flags any CSV venue absent from
// today_meetings so a slip-through at least produces a visible signal
// instead of silent failure in either direction.
function isNZMeeting(venue) {
  const v = (venue || '').toLowerCase().trim();
  return [...NZ_TRACKS].some(t => v === t || v.startsWith(t));
}

// Pure function — returns {allRaces, allVenues, raceKeys} with no side effects
// venuesWithData (optional): a Set of venues an independent, already-
// resolved source (e.g. race_cards) confirms have real data today -- when
// given, a bare venue name that's genuinely ambiguous (see
// AMBIGUOUS_VENUE_FALLBACKS in lib/venues.js, e.g. a raw CSV Meeting of
// "RANDWICK" that's actually today's Randwick Inside/Kensington meeting)
// gets resolved to its disambiguated sub-venue before the race key/venue
// grouping is built, so every downstream consumer (sidebar label, resulted-
// pill matching against race_results, pace-bias scoring) agrees with the
// rest of the site instead of only matching by coincidence. Omitted (the
// default) reproduces the exact prior behaviour -- existing callers that
// don't have this data available are unaffected.
export function buildRaces({ hdrs, rows }, venuesWithData = null) {
  const allRaces = {};
  const allVenues = {};
  ...
  rows.forEach(r => {
    ...
    if (!rnum) return;
    let venue0 = c(r, 'venue') || 'Unknown';
    if (isNZMeeting(venue0)) return;
    // Always canonicalise (e.g. "ILLAWARRA GRANGE" -> "KEMBLA GRANGE" via
    // VENUE_NORMALISE) so every consumer of allRaces/allVenues (Field tab,
    // Pace Map, LeftRail sidebar) shows the same venue name race_cards/
    // Value Bets does -- previously this only happened to fall out of the
    // ambiguous-fallback branch below when its result happened to differ
    // from the raw name, so any aliased venue with no AMBIGUOUS_VENUE_
    // FALLBACKS entry (e.g. Kembla Grange) kept showing its raw/alias CSV
    // spelling here indefinitely. resolveAmbiguousVenue still gets the
    // final say when real per-day data disambiguates a genuinely ambiguous
    // bare name (e.g. Caulfield -> Caulfield Heath) into a sub-venue.
    const canon = normaliseVenue(venue0);
    venue0 = venuesWithData ? resolveAmbiguousVenue(canon, venuesWithData) : canon;
    ...
    // Stable per-runner seed for deterministic jitter — same horse/race/date
    // always resolves to the same multiplier, no matter how many times the
    // CSV is re-parsed or the page reloaded that day.
    const jitterSeed = `${venue0}|${rnum}|${(c(r, 'horse') || '').toUpperCase()}|${c(r, 'date') || ''}`;

    const oddsNum = safeNum((c(r, 'odds') || '').trim());
    // Sanity cap is checked against the raw CSV value, before the reduction --
    // it exists to reject clearly-bogus source data (e.g. placeholder/blowout
    // odds), and a 26% cut would let some already-bogus values slip under 200.
    let oddsVal = oddsNum && oddsNum <= 200 ? Math.round(oddsNum * 0.74 * seededJitter(jitterSeed) * 100) / 100 : null;

    // Parsed unconditionally (not just inside the odds-placeholder branch
    // below) since scoring.js's 'debutpromise' Form-group factor also
    // needs the horse object to carry this regardless of whether the CSV
    // happened to have a market price.
    const trnOvrlWins   = safeNum(c(r, 'Trn Ovrl12m Wins'));
    const trnOvrlStarts = safeNum(c(r, 'Trn Ovrl12m Starts'));

    // First starters have no market price in the CSV at all. Rather than
    // leaving rawOdds/Ref $ blank, generate a placeholder from trainer overall
    // win% — tiered so a in-form trainer's debutant prices shorter than an
    // untried one. This placeholder is a deliberately-set estimate, not a raw
    // market price, so it does NOT go through the 26% reduction above.
    if (oddsVal === null && starts === 0) {
      // No trainer starts on record → treat as the weakest tier, same as a low win%.
      const trnWinPct = trnOvrlStarts > 0 ? (trnOvrlWins / trnOvrlStarts) * 100 : -1;
      let base;
      if (trnWinPct >= 14)        base = 6;
      else if (trnWinPct >= 11.6) base = 9;
      else if (trnWinPct >= 7.7)  base = 13;
      else                        base = 19;
      oddsVal = Math.round(base * seededJitter(jitterSeed) * 100) / 100;
    }
    ...
    allRaces[key].horses.push({
      ...
      'Career Prizemoney':  safeNum(c(r,'Career Prizemoney')),
      'BP':                 safeNum(c(r,'BP')),
      ...
      // Trainer's overall (not just local) 12-month win rate -- already
      // parsed above for the first-starter placeholder-odds calculation,
      // but never carried onto the horse object itself until now. Feeds
      // scoring.js's 'debutpromise' Form-group factor, the only current
      // consumer.
      trnOvrlWin: trnOvrlStarts > 0 ? trnOvrlWins / trnOvrlStarts : null,
      rawOdds: oddsVal,
