...
function isNZMeeting(venue, state) {
  const s = (state || '').trim().toUpperCase();
  if (s === 'NZ') return true;
  if (s) return false; // non-blank non-NZ state → definitively AU, skip name list
  // State absent/blank — fall back to venue name list
...
export function buildRaces({ hdrs, rows }) {
  ...
  rows.forEach(r => {
    ...
    if (isNZMeeting(venue0, c(r, 'state'))) return;
