...
import { isKnownAuVenue } from './venues';
...
// Allowlist-based: a meeting is included only if its venue resolves to a
// known Australian venue (lib/venues.js's AU_VENUE_STATE). Deliberately the
// inverse of the old NZ-name-blocklist approach -- a blocklist can only ever
// exclude tracks someone thought to enumerate, so an obscure/rarely-used
// meeting (NZ or otherwise) sails through unrecognised by default. Excluding
// by default means an unrecognised venue never silently appears, at the
// cost of needing AU_VENUE_STATE kept up to date -- see the console.warn
// below, which flags exactly that so a genuinely-new AU track doesn't just
// vanish without anyone noticing (same pattern as the backend's
// venue_has_mapping() / "UNMATCHED VENUE" warning in worker.py).
function isExcludedMeeting(venue) {
  if (isKnownAuVenue(venue)) return false;
  console.warn(`[csvParser] Excluding unrecognised venue "${venue}" -- not in AU_VENUE_STATE. If this is a real Australian venue, add it there; otherwise this is correctly filtering out a non-AU (e.g. NZ) meeting.`);
  return true;
}
...
export function buildRaces({ hdrs, rows }) {
  ...
  rows.forEach(r => {
    ...
    if (isExcludedMeeting(venue0)) return;
