...
// Display order + reverse lookup for pages rendering one column per
// PuntersEdge bookmaker (e.g. the Odds page) -- built from PUNTERSEDGE_SLUGS
// rather than hand-duplicated, so a new confirmed pair automatically gets a
// column with no second place to update.
export const PUNTERSEDGE_BOOKMAKER_COLUMNS = Object.entries(PUNTERSEDGE_SLUGS)
  .filter(([, slug]) => slug != null)
  .map(([name, slug]) => ({ name, slug }));

const SLUG_TO_NAME = Object.fromEntries(
  PUNTERSEDGE_BOOKMAKER_COLUMNS.map(({ name, slug }) => [slug, name]),
);

// Falls back to the raw slug if it's a PuntersEdge bookmaker we haven't
// mapped a display name for yet (odds_snapshot can carry slugs outside our
// own BOOKMAKERS list, e.g. bookmakers we don't offer for bet-logging).
export function bookmakerNameForSlug(slug) {
