  ...
  const perRace = await Promise.all(Object.values(races).map(async ({ venue, raceNum, horses }) => {
    ...
    scored.forEach((h, i) => {
      ...
      raceBets.push({
        ...
        // tab) -- getConfidenceTier still returns a correct answer from the
        // starts===0, calibration-bucket, and extreme-edge checks, just
        // without the sprint-specific compound case Field tab can flag.
        confidence: getConfidenceTier({ starts: h.starts, dist: null, calPrice: myOdds, oosMetrics, marketPrice }),
