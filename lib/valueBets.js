...
export async function fetchAllTodayValueBets(date) {
  ...
  Object.values(races).forEach(({ venue, raceNum, horses }) => {
    ...
    // approximate implementation. Once a race jumps, its CSV rawOdds
    // reflects a post-race market blowout rather than a genuine live
    // price. Filtered here, before any scoring work runs for the race, not
    // just at display time.
    if (hasRaceJumped(date, postTimeByRace[`${venue}||${raceNum}`])) return;
