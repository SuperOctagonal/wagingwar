...
// consecutive day; if a day was missed, a streak_shield (the wheel's
// "streak boost" prize) is auto-consumed to preserve the streak instead of
// resetting to 1, if one is available. Returns null if already claimed
// today (idempotent from the caller's perspective).
export async function claimDailyLogin(clerkId, { isPro = false } = {}) {
  ...
  const shieldUsed = !consecutive && account.last_daily_claim && (account.streak_shields || 0) > 0;
  const newStreak = (consecutive || shieldUsed) ? (account.login_streak || 0) + 1 : 1;
  ...
  await sb(`user_credits?clerk_id=eq.${encodeURIComponent(clerkId)}`, {
    ...
    body: {
      ...
      ...(shieldUsed ? { streak_shields: (account.streak_shields || 0) - 1 } : {}),
    },
  });
  return { awarded, balance, streak: newStreak, shieldUsed };
}
