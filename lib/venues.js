...
export const AU_VENUE_STATE = {
  ...
  'BIRDSVILLE':'QLD',
  // Added 2026-09-05, Tatura-missing-from-Results investigation: same
  // isKnownAuVenue() allowlist gap as BIRDSVILLE above, found again same
  // day -- re-running the same 60-day cross-reference (race_results/
  // race_schedule/today_meetings vs this allowlist) turned up four more
  // gaps beyond Tatura itself. States confirmed from today_meetings.state.
  'JUNCTION':'WA','KEMPSEY':'NSW','MENDOORAN':'NSW','PRAIRIE':'QLD','TATURA':'VIC',
};
...
export const AMBIGUOUS_VENUE_FALLBACKS = {
  ...
  // Confirmed live 2026-09-16: PuntersEdge labelled 6 of Caulfield Heath's
  // 8 races that day as bare "Caulfield" (only its feature race, R6, got
  // the full "Caulfield Heath") -- resolveAmbiguousVenue() had no entry to
  // redirect those 6 to the sub-venue race_cards actually stores them
  // under, so odds_snapshot silently never got rows for them all day.
  CAULFIELD: 'CAULFIELD HEATH',
};

// Shared resolver for AMBIGUOUS_VENUE_FALLBACKS -- takes an already-canonical
// venue name (post normaliseVenue()) plus a Set of venues an independent,
// already-resolved source (e.g. race_cards) confirms have real data today,
// and substitutes the fallback sub-venue only when the bare name has none of
// its own. Extracted from puntersedge-refs/route.js's original inline
// resolveRaceKey() logic so every caller applies the exact same rule instead
// of each maintaining its own copy.
export function resolveAmbiguousVenue(canonVenue, venuesWithData) {
  const fallback = AMBIGUOUS_VENUE_FALLBACKS[canonVenue];
  if (fallback && !venuesWithData.has(canonVenue) && venuesWithData.has(fallback)) {
    return fallback;
  }
  return canonVenue;
}
