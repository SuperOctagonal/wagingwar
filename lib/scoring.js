...
// Only dependency in this file -- a small, pure math utility (monotonic
// curve interpolation), not the calibration system itself (lib/
// calibrationApply.js, which depends on this one, not the other way
// round). Used solely by blendFirstStarterLivePrices' optional
// curvePoints param below.
...
export const MT = [
  ...
  // Same bucket boundaries as the individual Joc/Trn Loc12m Win % rows above
  // -- both are win-rate-on-[0,1] metrics scored the same way, no reason to
  // invent a different curve. See 'jtrat' factor below for the >=5-combined-
  // starts gate that keeps this null (no score contribution) below that.
  ['Score_Joc/Trn Win %',0.2,1,2],['Score_Joc/Trn Win %',0.16,0.19,1.8],
  ['Score_Joc/Trn Win %',0.12,0.15,1.6],['Score_Joc/Trn Win %',0.08,0.11,1.4],
  ['Score_Joc/Trn Win %',0.04,0.07,1],['Score_Joc/Trn Win %',0,0.03,0.8],
  // "Debut Promise" -- a small Form-group contribution for confirmed first
  // starters only (see the 'debutpromise' factor below). -1 is a sentinel
  // for "no trainer record at all" (trnOvrlWin null), scored the same as
  // the weakest known tier -- a modest floor, not zero, so a debutant
  // isn't dumped to the bottom of the field on missing data alone.
  //
  // Each tier is the original trainer-win%-tiered value (0.8-4.5) plus a
  // flat +6 baseline. First verification (af8a5b5) showed the tiered
  // value alone was too small to move rank in most fields -- the average
  // consecutive-rank score gap in an 18-19 runner field is ~2.4-2.9
  // points, bigger than the entire original 0.8-4.5 range, so all but the
  // strongest-trainer debutants barely moved. The flat +6 reflects the
  // calibration investigation's finding that first starters win roughly
  // 2x more often than their assigned price implies across every price
  // band tested (not just the extreme long end) -- "debutants as a whole
  // deserve more credit than zero" -- while the trainer tier is kept on
  // top of it to preserve relative differentiation between debutants.
  // Starting-point tuning, to be revisited against live market prices.
  ['Debut Promise',-1,-0.001,6.8], ['Debut Promise',0,0.0769,6.8],
  ['Debut Promise',0.077,0.1159,7.5], ['Debut Promise',0.116,0.1399,8.5],
  ['Debut Promise',0.14,0.1999,9.5], ['Debut Promise',0.2,1.1,10.5],
];
...
// Minimum combined jockey+trainer starts (PuntersEdge's own Joc/Trn Wins/
// Starts combo stat) before the 'jtrat' preview factor below trusts it --
// the edge-finding investigation found the underlying data too thin below
// this to mean anything. See 'jtrat' factor definition for full context.
export const JOC_TRN_MIN_STARTS = 5;
...
export const FACTORS = [
  ...
  // First-starter-only Form contribution -- real markets clearly price in
  // trainer confidence/stable support/trial form for debutants, which the
  // rest of the Form group (all null for starts=0) can't see at all.
  // Scores nothing (rawVal null) for any runner that has actually raced --
  // see getRawAndMetric's 'debutPromise' branch.
  { key:'debutpromise', label:'Debut Promise', grp:'Form',    derived:'debutPromise',            metric:'Debut Promise' },
  ...
  // Admin-only preview (see RunnerRow's jocTrnPreview block, app/races/
  // page.js) -- NOT in GRP_KEYS/FACTOR_GROUPS_DEF's live 'conn' group list
  // below, so it never contributes to totalFromGroups/myOdds for any
  // regular Pro user. Edge-finding investigation (2026-09-18) found a
  // clean, out-of-sample-validated gradient using PuntersEdge's own
  // Joc/Trn combo stat (jocTrnWins/jocTrnStarts, captured since the CSV
  // parser's beginning but never scored) -- but ALSO found the gradient is
  // concentrated almost entirely in the top individual-skill tercile
  // (Q4-Q1 gradient ~5.4pp there, OOS-confirmed) and is flat-to-nil for
  // below-average jockeys/trainers (~0pp) -- i.e. it's not a uniform
  // "combo bonus," it mostly matters for already-strong combinations. A
  // flat weight=10 factor doesn't capture that shape, which is exactly why
  // this ships as a preview to look at, not straight into live scoring.
  // scoreZero:true is the second (belt-and-braces) guard: it's what keeps
  // this out of scoreHorse()'s total too -- scoreHorse iterates ALL of
  // FACTORS regardless of FACTOR_GROUPS_DEF group membership, and is used
  // live for every user's Competitions score (app/competitions/page.js),
  // not just Field tab's scoreGroup path. Being absent from FACTOR_GROUPS_
  // DEF's 'conn' list is enough to keep scoreGroup from touching it, but
  // scoreZero is what keeps scoreHorse's Competitions total from silently
  // picking it up at the default weight=10 the same way the 'odds'
  // reference factor above is deliberately excluded from its own total.
  { key:'jtrat',     label:'Joc/Trn Combo', grp:'Connections', derived:'jocTrnWin',             metric:'Score_Joc/Trn Win %', scoreZero:true },
...
export const FACTOR_GROUPS_DEF = [
  { key:'form',  label:'Form',        color:'#d97706', factors:[
    ...
    {key:'avgfin',label:'Avg Finish'},{key:'debutpromise',label:'Debut Promise'},
  ]},
...
function getRawAndMetric(h, f, trackCond) {
  ...
  else if (f.derived === 'jocTrnWin') {
    // Below JOC_TRN_MIN_STARTS combined starts, the edge-finding
    // investigation found this stat too thin to trust (same discipline as
    // MIN_TEST_N elsewhere in the codebase) -- null here means "no score
    // contribution," identical to any other inapplicable factor, not a
    // misleading default like 0%.
    rawVal = h.jocTrnStarts >= JOC_TRN_MIN_STARTS ? h.jocTrnWins / h.jocTrnStarts : null;
  }
  else if (f.derived === 'debutPromise') {
    // Only a confirmed first starter gets a value here at all -- an
    // experienced runner gets rawVal null, so this factor contributes
    // nothing for them, same as any inapplicable factor. -1 is the
    // "no trainer record" sentinel matched against MT's Debut Promise
    // rows (see there for why this can't just be a null-skip like every
    // other factor: the floor score needs to apply even when trnOvrlWin
    // itself is unknown).
    rawVal = Number(h.starts) === 0 ? (h.trnOvrlWin ?? -1) : null;
  }
  else if (f.key === 'days') {
    // A true first starter (starts=0) has "Days Since Last Start" encoded
    // as 0 in the CSV -- identical to what a horse that raced literally
    // today would show, because there's no real value to put there.
    // Reading that 0 literally previously matched mtLookup's [0,14,3]
    // band, the BEST possible recency score, purely from this encoding
    // collision. Excluded (null) for a confirmed debutant instead, same
    // as every other null Form/Speed input for a first starter already
    // contributes zero rather than a false-positive score.
    rawVal = Number(h.starts) === 0 ? null : h[f.csvCol];
  }
...
// Placegetter points for the Pace Bias panels (Results + Races pages) --
// 1st/2nd/3rd only, everything else scores 0. Shared so the 3/2/1 weighting
// can't drift between the two pages' independent data-gathering.
export function pointsForPlace(place) {
  return place === 1 ? 3 : place === 2 ? 2 : place === 3 ? 1 : 0;
}
...
export function calcPaceMap(horse, raceVenue, raceDist, trackCond) {
  ...
  // Leader badge only shown when pace score >= 78%; below that, reclassify as Midfield
  if (role === 'Leader' && pct < 78) {
    role  = 'Midfield';
    color = '#ffc000';
  }
...
// PM has no hand-tuned rows for field sizes 21-23 (only 2-20, then a jump
// to 24) -- getMatrixRange used to fall back to PM[16] for these three
// sizes via `PM[key] || PM[16]`, which is wrong: a 21-23 runner field's
// tail should price longer than a 16-runner field's, not identically.
// Filled in by linearly interpolating between the neighbouring 20- and
// 24-runner rows -- by fractional rank position, since the two tables
// have different row counts -- rather than leaving the gap or hand-typing
// guessed values.
(function fillPmFieldSizeGaps() {
  const lo = PM[20], hi = PM[24];
  for (let n = 21; n <= 23; n++) {
    const t = (n - 20) / (24 - 20); // 0..1 blend weight toward the 24-runner table
    const rows = [];
    for (let rank = 1; rank <= n; rank++) {
      const frac = n > 1 ? (rank - 1) / (n - 1) : 0; // 0 (top rank) .. 1 (last rank)
      const loRow = lo[Math.round(frac * (lo.length - 1))].split('-').map(parseFloat);
      const hiRow = hi[Math.round(frac * (hi.length - 1))].split('-').map(parseFloat);
      const min = loRow[0] + (hiRow[0] - loRow[0]) * t;
      const max = loRow[1] + (hiRow[1] - loRow[1]) * t;
      rows.push(`${min.toFixed(2)}-${max.toFixed(2)}`);
    }
    PM[n] = rows;
  }
})();
...
function getMatrixRange(fieldSize, rank) {
  ...
  const rows = PM[key];
  if (rank > rows.length) {
    // Every PM row set tops out at its own last rank (matching the field
    // size it was built for). Previously any rank beyond that used a
    // single flat [51, 181] regardless of which table was selected, so a
    // rank-25 runner in a 30-horse field and a rank-25 runner in a
    // 40-horse field (both clamped to the same 24-runner table) got an
    // identical, non-scaling range. Extrapolates instead from the growth
    // rate between that table's own last two rows, continued forward by
    // however many ranks past the table's end this one is -- ties the
    // overflow range to the field-size table actually selected rather
    // than a constant. (Still shared across every field size that clamps
    // to the same table, e.g. all fieldSize>=24 -- finer per-field-size
    // resolution beyond the table's own range is the larger probability-
    // based model discussed separately, not this fix.)
    const lastIdx = rows.length - 1;
    const [lastMin, lastMax] = rows[lastIdx].split('-').map(parseFloat);
    const [prevMin, prevMax] = rows[lastIdx - 1].split('-').map(parseFloat);
    const growthMin = prevMin > 0 ? lastMin / prevMin : 1.15;
    const growthMax = prevMax > 0 ? lastMax / prevMax : 1.15;
    const stepsBeyond = rank - rows.length;
    return [
      lastMin * Math.pow(growthMin, stepsBeyond),
      lastMax * Math.pow(growthMax, stepsBeyond),
    ];
  }
...
// Blends a confirmed first starter's rank-determining score with its live
// market price, once one exists -- live-matched data showed trainer win%
// (the 'debutpromise' factor above) has almost no correlation with actual
// debutant pricing (R²=0.059 across 22 matched runners), while the live
// market clearly has real information this pipeline otherwise can't see
// at all for a starts=0 runner. Once a price exists, it should dominate
// the runner's rank -- not just be a display-only override after the
// fact, since rank has to stay internally consistent with price for
// every runner (including whichever other runners a promoted/demoted
// first starter displaces).
//
// This has to run BEFORE the real calculateMatrixOdds call that produces
// final prices, so it runs its own throwaway pass first: score the field
// as normal (already done by the caller) and run calculateMatrixOdds
// once just to get a provisional price for every horse, purely as the
// "model-implied probability" input to the blend and as the reference
// curve used to convert a blended price back into an equivalent
// totalFromGroups value (see priceToScore below) -- there's no other way
// to get a probability out of a raw score, since the matrix's price
// range depends on rank position, not a standalone formula.
//
// flags: the same odds_snapshot "current best price" map
// fetchMarketMoveFlags() (lib/marketMoves.js) already returns --
// {[nameKey]: {current, open, move}} -- reused as-is rather than a new
// fetch path, per nameKeyFn (pass marketMoves.js's own `nameKey`).
//
// curvePoints (optional): the active Phase 2 calibration curve's knot
// array. When supplied, the "modelProb" side of the blend (and the
// reference curve used to convert a blended price back onto the
// totalFromGroups scale) uses the CALIBRATED provisional price, not the
// raw one -- once calibration is the live WW $ for everyone, a first
// starter's blend has to weigh the live price against the SAME price a
// user actually sees, not an internal pre-calibration number. Omitted
// (the default) by every training-data builder (lib/calibrationSample.js,
// lib/trustSample.js, lib/raceFeatureSnapshots.js) that still needs the
// untouched raw price -- this stays fully backward compatible.
export const FIRST_STARTER_LIVE_WEIGHT = 0.8; // vs 0.2 on the existing model score. Starting point -- a natural future tunable, hardcoded for now.

function calibratePriceInline(price, curvePoints) {
  if (!(price > 0) || !curvePoints?.length) return price;
  const prob = interpolateCurve(curvePoints, 1 / price);
  return prob > 0 ? 1 / prob : price;
}

export function blendFirstStarterLivePrices(scoredHorses, flags, nameKeyFn, curvePoints = null) {
  if (!scoredHorses.length) return scoredHorses;
  const hasAnyFirstStarterWithLivePrice = scoredHorses.some(h =>
    Number(h.starts) === 0 && flags?.[nameKeyFn(h.name)]?.current > 0
  );
  if (!hasAnyFirstStarterWithLivePrice) return scoredHorses; // fallback: nothing to blend, field unchanged

  const provisional = [...scoredHorses].sort((a, b) => b.totalFromGroups - a.totalFromGroups);
  const rawProvisionalPrices = calculateMatrixOdds(provisional);
  const provisionalPrices = curvePoints?.length
    ? rawProvisionalPrices.map(p => calibratePriceInline(p, curvePoints))
    : rawProvisionalPrices;

  // Reference curve: (price, totalFromGroups) pairs from every horse NOT
  // being blended -- used to convert a blended price back onto the same
  // score scale as the rest of the field, by interpolating where the
  // blended price falls among prices the rest of the field already
  // earned honestly. Sorted ascending by price.
  const referenceCurve = provisional
    .map((h, i) => ({
      price: provisionalPrices[i],
      score: h.totalFromGroups,
      isBlended: Number(h.starts) === 0 && flags?.[nameKeyFn(h.name)]?.current > 0,
    }))
    .filter(p => !p.isBlended)
    .sort((a, b) => a.price - b.price);

  function priceToScore(price) {
    if (!referenceCurve.length) return null;
    if (price <= referenceCurve[0].price) return referenceCurve[0].score;
    const last = referenceCurve[referenceCurve.length - 1];
    if (price >= last.price) return last.score;
    for (let i = 0; i < referenceCurve.length - 1; i++) {
      const a = referenceCurve[i], b = referenceCurve[i + 1];
  ...
  const blended = provisional.map((h, i) => {
    if (Number(h.starts) !== 0) return h;
    const livePrice = flags?.[nameKeyFn(h.name)]?.current;
    if (!(livePrice > 0)) return h; // no live price for this runner yet -- fallback, unchanged
    const provisionalPrice = provisionalPrices[i];
    const liveProb = 1 / livePrice;
    const modelProb = 1 / provisionalPrice;
    const blendedProb = FIRST_STARTER_LIVE_WEIGHT * liveProb + (1 - FIRST_STARTER_LIVE_WEIGHT) * modelProb;
    const blendedPrice = 1 / blendedProb;
    const blendedScore = priceToScore(blendedPrice);
    if (blendedScore == null) return h;
    return { ...h, totalFromGroups: blendedScore };
  });

  // Re-sort by the (now possibly-changed) totals -- lets the blended
  // runner compete for its real rank, and any other runner it passes or
  // falls behind shifts accordingly. A final calculateMatrixOdds call on
  // this returned order (the caller's job, same as any other race) is
  // what actually produces the real, final prices.
  return blended.sort((a, b) => b.totalFromGroups - a.totalFromGroups);
}

// Value edge: how much better (or worse) the market price is than the
// model's own price (WW $) -- positive means the market is offering longer
// odds than the model thinks it should (a value opportunity), negative
// means the market has shortened past what the model justifies. Extracted
// from RunnerRow/MobileRunnerCard's original inline VALUE-column formula so
// the Value Bets tab (app/api/value-bets, computed server-side across all
// of today's races) and the Field tab's own VALUE column always agree on
// the exact same calculation -- never two copies of the same formula that
// could drift apart.
//
// Note: calculateMatrixOdds() above has a deliberate small random jitter in
// its own odds, so myOdds (and therefore this edge %) is not perfectly
// bit-identical between two independent computations of the same race (one
// on the Field tab, one in the Value Bets batch) -- only the formula itself
// is guaranteed identical, not byte-for-byte output. The Value Bets tab
// also scores with getDefaultWeights() rather than a signed-in user's own
// WeightsPanel customisations (which live client-side only), so its edge %
// can differ slightly from a Field tab session with non-default weights.
export function computeValueEdge(marketPrice, modelPrice) {
  if (!marketPrice || !modelPrice) return null;
  const pct = (marketPrice - modelPrice) / modelPrice * 100;
  const arrow = pct >= 30 ? '▲' : pct <= -30 ? '▼' : '';
  const str = `${arrow}${pct >= 0 ? '+' : ''}${pct.toFixed(0)}%`;
  const color = pct >= 20 ? '#059669' : pct <= -20 ? '#dc2626' : '#374151';
