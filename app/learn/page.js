...
const SECTIONS = [
  {
    ...
    bullets: [
      ...
      'Ref $ — estimated price from the race fields',
      'Value % — gap between Edge and Ref. Green = value bet, red = overbet',
  ...
  },
  {
    ...
    title: 'Edge $ & Value %',
    live: true,
    desc: "Edge $ is our model's fair value price for each runner, calculated from their score relative to the field. Value % shows how that compares to the estimated price from the race fields.",
    bullets: [
      'Edge $ — what our model thinks the horse should pay',
      'Ref $ — estimated price from the race fields',
      'Value % — (Ref - Edge) / Edge × 100. Positive = the field price is higher than the model price',
      '+30% or higher = strong value. Look for rank 1 or 2 horses with high value %',
      'Negative value = the field price is shorter than the model price — may be overbet',
