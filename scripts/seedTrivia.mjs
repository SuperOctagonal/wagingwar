// One-time (or re-run-to-append) seed script for trivia_questions.
// Usage: node scripts/seedTrivia.mjs
// Reads Supabase creds from .env.local directly (not loaded via Next.js).
import fs from 'fs';
import { TRIVIA_SEED } from '../lib/gamesTriviaSeed.js';
...
const res = await fetch(`${SURL}/rest/v1/trivia_questions`, {
  ...
  body: JSON.stringify(TRIVIA_SEED.map(q => ({ ...q, active: true }))),
});
