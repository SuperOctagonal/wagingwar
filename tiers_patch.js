const fs = require('fs');
let c = fs.readFileSync('app/community/page.js', 'utf8');

const newCode = `function getTier(pts) {
  pts = pts || 0;
...
// Find and replace TIER_GROUPS array
const start = c.indexOf('const TIER_GROUPS = [');
const end = c.indexOf('];', start) + 2;
if (start === -1) { console.log('TIER_GROUPS not found'); process.exit(1); }
c = c.slice(0, start) + newCode + c.slice(end);
fs.writeFileSync('app/community/page.js', c);
